...
import { getTokenDesc } from './getTokenDesc.mjs';
...
function parseObjectTypeExtension(lexer) {
  ...
  if (
    interfaces.length === 0 && directives.length === 0 && fields.length === 0
  ) {
    throw unexpected(lexer);
  }
...
function parseDirectiveLocation(lexer) {
  const start = lexer.token;
  const name = parseName(lexer);
  if (DirectiveLocation.hasOwnProperty(name.value)) { return name; }
 ...
 * return true after advancing the lexer. 
...
function expectKeyword(lexer, value) {
  if (!skipKeyword(lexer, value)) {
    throw syntaxError(
      lexer.source, 
      lexer.token.start, 
      "Expected \"".concat(value, "\", found ").concat(getTokenDesc(lexer.token))
    );
  }
}

/**
 * Creating an error when an unexpected lexed token is encountered.
 */
 ...
 * This list begins with a lex token of openKind
 * and ends with a lex token of closeKind. 
...
function any(lexer, openKind, parseFn, closeKind) {
  ...
  const nodes = [];
  while (!skip(lexer, closeKind)) { nodes.push(parseFn(lexer)); }
  return nodes;
}
